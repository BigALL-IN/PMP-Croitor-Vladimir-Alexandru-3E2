...
def main():
    ...
    x_bar = data.mean()
    print(f"Sample mean = {x_bar}, Sample std = {data.std(ddof=1)}")
    # a, b)
    ...
    print(f"SD:   {np.std(data, ddof=1)}")
    print("The Bayesian and frequentist estimates are very similar, thanks to a weak prior and sufficient data, the small difference is caused by bayes being influenced by the small prior")
    # d)
    ...
    print("The strong prior shifts the HDI towards 50dB, while in the case of the weak prior sigma mainly relies on data, since HalfNormal(10) is broad")
